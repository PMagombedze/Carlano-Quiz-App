...
@linux.route("/api/v1/linux/add", methods=["POST"])
@basic_auth.required
def add_question():
...
@linux.route("/api/v1/linux/", methods=["GET"])
@basic_auth.required
def get_questions():
    questions = load_questions()
...
def quiz():
    ...
    if request.method == "POST":
        ...
        return render_template("quizzes/python.html", question=questions[question_index], question_index=question_index, score=session["score"], total=session["total"], course="Linux")

    session["score"] = 0
    session["total"] = 0
    questions = load_questions()
    return render_template("quizzes/python.html", question=questions[0], question_index=0, score=session["score"], total=session["total"], course="Linux")
